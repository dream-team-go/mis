...
public class ShiroPlugin implements IPlugin {
	...
	public boolean start() {
		...
		//ConcurrentMap，它是一个接口，是一个能够支持并发访问的java.util.map集合 比hashtable效率高
	...
	private void scanAnnotation(List<AuthzHandler> authzArray,
		...
		for (Annotation a : annotations) {
			if (a instanceof RequiresRoles) {//角色认真
				authzArray.set(0, new RoleAuthzHandler(a));
			} else if (a instanceof RequiresPermissions) {//权限认证
				authzArray.set(1, new PermissionAuthzHandler(a));
			} else if (a instanceof RequiresAuthentication) {//身份认证（验证用户是否登录）
				authzArray.set(2, AuthenticatedAuthzHandler.me());
			} else if (a instanceof RequiresUser) {//用户认证（验证用户是否被记忆）
				authzArray.set(3, UserAuthzHandler.me());
			} else if (a instanceof RequiresGuest) {//验证是否是一个guest的请求，与@RequiresUser完全相反。（代表无需认证即可访问）
